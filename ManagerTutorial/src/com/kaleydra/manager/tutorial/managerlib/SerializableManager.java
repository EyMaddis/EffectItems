...
/*
 * if you read this for the serialization tutorial, just read the commented part
 */
	...
	public boolean load(@Nonnull File ymlFile, FileFilter filter){
		...
		for(File file: files){
			try {
				...
				// load from file, will deserialize everything automatically
				yml.load(file);
	...
	public boolean save(@Nonnull File ymlFile, FileNameGenerator<Type> namer){
		...
		for(File file:files){
			...
			try {
				...
				// save the data at "storage"
				yml.set(YAML_KEY, data.values().toArray());
				
				// will save to file and serialize everything.
				yml.save(file);
